...
- `redirect_when_blocked` on the `surveyJS` style: when a save is refused, follow `redirect_at_end` instead of showing "Data not saved!". Off by default, so a failed save stays visible as before and no existing survey changes. Intended for studies using `block_updates_when`, where a resubmission under an already finished key is refused on purpose and the participant should simply be sent on.
- `block_updates_when` on the `surveyJS` style: names a column that locks a row once it is set. Empty (the default) never locks. Set to a column name and a row whose value there is set and not "0" refuses further writes, so a resubmission under an already-used key cannot overwrite finished data. A survey that asks the column itself is never blocked by it, so the survey that finishes a run can still save. Pairs with `update_based_on`: one names the key, the other makes it single-use. The check runs in the model, so it holds for a direct POST as well as a normal submission.
